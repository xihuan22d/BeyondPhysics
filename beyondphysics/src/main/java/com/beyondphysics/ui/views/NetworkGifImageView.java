...
public class NetworkGifImageView extends AppCompatImageView {
    ...
    private final Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0:
                    if (gifBitmaps.size() > showGifPosition) {
                        GifBitmap gifBitmap = gifBitmaps.get(showGifPosition);
                        int delay = gifBitmap.getDelay();
                        Bitmap bitmap = gifBitmap.getBitmap();
                        setImageBitmap(bitmap);
                        Message message = new Message();
                        message.what = 0;
                        handler.sendMessageDelayed(message, delay);
                        showGifPosition = showGifPosition + 1;
                    } else {
                        if (gifLoop) {
                            showGifPosition = 0;
                            if (gifBitmaps.size() > showGifPosition) {
    ...
    public NetworkGifImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void doBitmapRequest(BitmapRequest_Default_Params bitmapRequest_Default_Params, boolean canFilter) {
        ...
        if (canFilter && openSameKeyFilter && !showGif && !haveChangeBitmap && oldBitmapRequest_Default_Params != null) {
                ...
                public void onSuccessResponse(BitmapResponse response) {
                    if (response != null && response.getSuperBitmap() != null) {
                        ...
                        if (!superBitmap.isDecodeGif()) {
                            setImageBitmap(superBitmap.getBitmap());
                        } else {
                            showGif = true;
                            showGif(superBitmap.getGifBitmaps());
                        }
    ...
    private void showGif(List<GifBitmap> gifBitmaps) {
        if (gifBitmaps == null) {
            return;
        }
        this.gifBitmaps = gifBitmaps;
        showGifPosition = 0;
        if (this.gifBitmaps.size() > showGifPosition) {
            GifBitmap gifBitmap = gifBitmaps.get(showGifPosition);
            int delay = gifBitmap.getDelay();
            Bitmap bitmap = gifBitmap.getBitmap();
            setImageBitmap(bitmap);
    ...
    protected void onAttachedToWindow() {
        ...
        if ((showGif || !haveGetBitmap) && cancelRequestWhenOnDetachedFromWindow && fixedRecyclerViewBug && !fromOnBindViewHolder && oldBitmapRequest_Default_Params != null) {
